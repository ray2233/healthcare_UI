...
Healthcare.Helpers.tileManagerHelper = function () {
    ...
    var showTileManager = function (index) {
        var $mainPanel = $(mainPanelSelector),
            animDur = $tileManagerContainer.igTileManager("option", "animationDuration");
        ...
        $tileManagerContainer.igTileManager({
            ...
            tileMaximizing: function (evt, ui) {
                ...
                if ($("#appointment-details").length > 0) {
                    $("#appointment-details").remove();
                    $(".details-overlay").remove();
                }

                if ($("#medication-details").length > 0) {
                    $("#medication-details").remove();
                    $(".details-overlay").remove();
                }
            },
...
Healthcare.Models.dashboard = function (dashboard) {
    //private methods
    var getRecentAppointments = function () {
        var numberOfRecentAppointments = 5,
            indexOfFirstRecentAppointment = dashboard.appointmensData.length - numberOfRecentAppointments;
        return dashboard.appointmensData.slice(indexOfFirstRecentAppointment, dashboard.appointmensData.length);
    };

    var getUpdatedAppoinmentData = function () {
        var numberOfAppointmentToDisplay = 10,
            indexOfFirstRecentAppointment = dashboard.appointmensData.length - numberOfAppointmentToDisplay;
    ...
    var getDashboardData = function () {
        dashboard.recentAppointments = getRecentAppointments();
        dashboard.appointmensData = getUpdatedAppoinmentData();
        return dashboard;
    };
...
Healthcare.Core.Pages.dashboard = function () {
    var $medicationDetails,
        ...
        menuPanelSelector = "#menu-panel-dashboard",
    ...
    var detailsView = function () {
        ...
        var appendAppointmentDetailsView = function () {
            var clickedAppointment = dashboardData.appointmensData[dataIndex],
    ...
    var bindEvents = function () {
        ...
        $(appointmentsViewSelector).on("iglistitemrendered", function (evt, ui) {
            ui.html = $.ig.tmpl('<li class="appointment ui-li-static ui-body-c{EXTRACSS}" data-idx="' + ui.index + '" data-id="${id}" data-theme="c">' +
                "<div class='appointments-left-section'>" +
                    "<div class='appointments-date'>" + ui.item.date.getDay() + "." + (ui.item.date.getMonth() + 1) + "." + ui.item.date.getFullYear() % 100 + "</div>" +
        ...
        $document.on("pagebeforechange", function (e, data) {
            if ($tileManagerContainer !== undefined) {
                var index,
        ...
        $document.on("pagechange", function (e, data) {
            if ($tileManagerContainer !== undefined && data.options.link) {
                // Close the menu panel
                $(menuPanelSelector).panel("close");
            }
        });
